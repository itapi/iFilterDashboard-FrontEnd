import { ArrowRight, User, Crown, Smartphone, CreditCard, CheckCircle, X, Zap, UserX, Settings, Radio, Terminal, MessageCircle } from 'lucide-react'
...
const ClientDetailsHeader = ({ client, onBack, activeTab, onTabChange, isCustomPlan, onStartLiveSession }) => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${config.color}`} title={daysRemainingText || ''}>
        <Icon className="w-4 h-4 ml-1" />
        {config.label}
        {daysRemainingText && <span className="mr-1 text-xs opacity-75">({daysRemainingText})</span>}
      </span>
    )
  }
